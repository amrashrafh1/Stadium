...
class AuthController extends Controller
{
    ...
    public function register(RegisterRequest $request)
    {
        ...
        try {
            ...
            DB::commit();
            return $this->successResponse(['user' => new \App\Http\Resources\API\Client\AuthResource($user)], __('auth.welcome'));
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function verifyAccount(\App\Http\Controllers\Api\Client\VerifyOtpRequest $request)
    {
        ...
        try {
            ...
            DB::commit();
            return $this->successResponse(['user' => new AuthResource($user), 'token' => $token], __('auth.welcome'), 200);
        } catch (\Exception $e) {
            DB::rollback();
